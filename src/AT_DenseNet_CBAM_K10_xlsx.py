...
from keras.layers import Conv2D, GlobalAveragePooling2D,BatchNormalization,Dropout,add,AveragePooling2D
...
from models import  utils, get_data, models_train, sound_test_finalizado, maneger_gpu
...
def hyper_model(config_model):
    ...
    conv = base_model.layers[-28].output

    # Adding CBAM after the last convolutional block
    cbam_output = CBAM(conv, config_model['ks'], config_model['r'])
    
    # Classification Head
    output = GlobalAveragePooling2D()(cbam_output)
    output = layers.Dense(config_model['num_classes'], activation='softmax')(output)
    model = Model(inputs=base_model.input, outputs=output)
    
    # Unfreeze layers based on the 'freeze' index for fine-tuning
    freeze = int(config_model['freeze'])
...
def sheet_exists(path_xlsx):
    ...
    try:
        ...
        cols_exe = [
            ...
            'duration', 'best_epoch','num_epoch','algorithm'
        ]
...
def save_results(row, k, val_loss, val_accuracy, me, res_train, path_xlsx):
    ...
    data = [
        ...
        res_train['best_epoch'], res_train['num_epochs'],  
        script_name
    ]
...
def execute_test(params, tests, test_index, start_index, num_partitions, start_k, save_dir, path_xlsx):
    ...
    for partition_index in range(num_partitions):
        ...
        model_tl, val_loss, val_accuracy, res_train = process_train(config_train)
